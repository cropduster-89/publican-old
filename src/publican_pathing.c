...
*
*/
static inline bool CanLeaveStep(struct tile_data new,
...
*
*/
static inline bool CanEmbarkStep(struct tile_data new,
				 struct tile_data currentNode,
				 enum direction dir,
				       ...
				       struct point3 pos)
{
	...
	return((c.walled == 0 || c.walled == 1) && !n.impassable &&
	       ...
	       (CanEmbarkStep(n, c, dir, floorFlag) && c.elevation == n.elevation) ||
	       (CanEmbarkStep(n, c, dir,floorFlag) && CanLeaveStep(n, c, dir)) ||
	       (CanLeaveStep(n, c, dir))));
}
				      ...
				      struct point3 pos)
{
	...
	return((e.walled == 0 || e.walled == 2) && !e.impassable &&
	       ...
	       (CanEmbarkStep(e, c, dir, floorFlag) && c.elevation == e.elevation) ||
	       (CanLeaveStep(e, c, dir) && CanLeaveStep(e, c, dir)) ||
	       (CanLeaveStep(e, c, dir))));
}
				       ...
				       struct point3 pos)
{
	...
	return((s.walled == 0 || s.walled == 1) && !s.impassable &&
		...
		(CanEmbarkStep(s, c, dir, floorFlag) && c.elevation == s.elevation) ||
		(CanEmbarkStep(s, c, dir, floorFlag) && CanLeaveStep(s, c, dir)) ||
		(CanLeaveStep(s, c, dir))));
}
				      ...
				      struct point3 pos)
{
	...
	return((c.walled == 0 || c.walled == 2) && !w.impassable &&
	       ...
	       (CanEmbarkStep(w, c, dir, floorFlag) && c.elevation == w.elevation) ||
	       (CanEmbarkStep(w, c, dir, floorFlag) && CanLeaveStep(w, c, dir)) ||
	       (CanLeaveStep(w, c, dir))));
}
...
*/
static enum direction GetDirectionFromCoord(struct node_list *destNode,
...
*		from the input pos
*/
static struct point3 GetCoordFromDirection(int32_t dir,
					   struct point3 pos,
					   enum floor_flag floorFlag)
{
				      ...
				      struct node_list *path)
{
	...
	while(currentNode != endNode)
	{
		...
		{
			testCoords = GetCoordFromDirection(dir, currentNode->pos, floorFlag);
	...
	*/
	for(int32_t i = 0; i < 128; ++i)
	{
		...
		if(i != 0) {
			path[i].dir = GetDirectionFromCoord(&path[i - 1], &path[i]);
		} else {
			path[i].dir = DIR_NULL;
		}
