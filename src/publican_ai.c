...
static inline bool IsBeingServed(struct entity_char *chara)
{
	...
	if(bar->users[0].ent) {
		...
		struct entity_char *bartender = GET_CHAR(bar->users[0].ent);
		assert(bartender->stats.isStaff);
			
		if(bartender->target.ent == chara->target.ent  &&
		   bartender->intent.type == intent_serve && 
		   bartender->intent.serve.stage == serve_sell) {
			result = true;
		}
	}
	return(result);
...
static void Serve(struct world_mode *world,
		  struct entity_char *bartender)
{
	assert(bartender->intent.type == intent_serve);
	
	struct char_intent *intent = &bartender->intent;
	...
	case serve_init: {
		...
		entity_Move(bartender, world);
		if(!bartender->activePath) {						
			intent->serve.stage = serve_sell;
			GET_CHARBASE(world, bartender)->rotation = SetBarFacing(world, bartender);
			ai_StartTimedEvent(world, bartender);	
		}
		break;
	} case serve_sell: {		
		struct entity_furn *furn = GET_FURN(bartender->target.ent);
		if(ai_EndTimedEvent(world, bartender)) {
			if(!furn->users[1].ent) {
				bartender->intent.type = intent_null;	
				UNSET_TARGET(world, bartender);		
			}
		}		
		break;
	} case serve_wait: {		
		if(ai_CheckBar(world, bartender)) {
						
			struct point3 currentPos = Vec3ToPoint3(
				GET_CHARBASE(world, bartender)->pos.xyz);
			struct point3 targetPos = ai_GetBarSlotPos(
				world, bartender, GET_TARGETFURN(world, bartender));
			
			if(CompPoint3(targetPos, currentPos)) {					
				intent->serve.stage = serve_sell;
			} else {				
				bartender->currentNode = path_GetPath(currentPos, targetPos,
					bartender, world, bartender->path);	
				if(!bartender->currentNode) {
					bartender->intent.type = intent_null;
					UNSET_TARGET(world, bartender);
#ifdef DEBUG
					printf("%s %s cannot find path to bar\n", 
						bartender->stats.firstName,
						bartender->stats.lastName);
#endif								
					break;
				} else {
					intent->serve.stage = serve_walk;				
					bartender->activePath = 1;
				}				
			}					
		}
		break;
